        ...
        If Browser_Playing AndAlso Not Browser_Loaded AndAlso e.Frame.IsMain Then
            ...
            Else
                ...
                With e.Browser
                    ...
                    '播完暂停
                    .EvaluateScriptAsync("const interval2 = setInterval(() => document.getElementsByName('bui-radio1')?.forEach((item) => { if (item.value === '2') { item.click(); clearInterval(interval2); } }), 500);")
                    ...
                    '开启声音
                    .EvaluateScriptAsync("const interval4 = setInterval(() => { let group = document.getElementsByClassName('bpx-player-ctrl-volume-icon'); if (group.length > 0) { if (group[0].style['display'] == 'none') { group[0].click(); setTimeout(() => easy_k.tryClick(), 500)} clearInterval(interval4); }  }, 500);")
                End With
            End If
        End If
    End Sub
