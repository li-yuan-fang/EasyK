...
Public Class FrmPlayer
    ...
    Public Sub New(K As EasyK, Settings As SettingContainer)
        ...
        Dim BrowserSettings As New RequestContextSettings() With {
            .CachePath = CachePath,
    ...
    Private Sub RunBiliScript(Browser As IBrowser)
        With Browser
            ...
            '打开声音(Win7专用)
            .EvaluateScriptAsync("const interval5 = setInterval(() => { let group = document.getElementsByClassName('bpx-player-ctrl-volume-icon'); if (group.length > 0) { if (group[0].style['display'] == 'none') { group[0].click(); setTimeout(() => easy_k.tryClick(), 500)} clearInterval(interval5); }  }, 50);")
